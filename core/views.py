...
def csv_record(request):
    ...
    records = Record.objects.all()
    writer.writerow(['Date Created', 'First Name', 'Last Name', 'Address', 'Email', 'Phone'])
    for record in records:
        writer.writerow([record.created_at.strftime("%d-%b-%Y"), record.first_name, record.last_name, record.address, record.email, record.phone])
    return response
